...
and writes hive-manifest.json. Schema v2.0 frozen.
"""
...
from hivemind.fs.frontmatter import parse_frontmatter
...
def _walk_md_files(directory: Path, root: Path) -> list[Path]:
    ...
    for entry in entries:
        ...
        # .github is the one dot-dir kept: repo-surface docs stay indexed
        # (is_system_note classifies them as system, never as content).
        if entry.name != ".github" and (
            entry.name in EXCLUDE_DIRS or entry.name.startswith(".")
        ):
            continue
        if entry.is_dir():
...
def build_manifest(hive_path: Path) -> dict[str, Any]:
    ...
    md_files = _walk_md_files(hive_path, hive_path)
    ...
    for full_path in md_files:
        ...
        rel_path = full_path.relative_to(hive_path).as_posix()
    ...
    return {
        ...
        "vault_path": str(hive_path).replace("\\", "/"),
...
def build_and_write_manifest(hive_path: Path) -> str:
    manifest = build_manifest(hive_path)
    manifest_path = hive_path / "_meta" / "hive-manifest.json"
